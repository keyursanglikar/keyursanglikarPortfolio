...
export default Tabs;`,
    ...
    "Modal Dialog": { label: "Modal Dialog ⚛️", language: "react", jsx: `// Modal Dialog
import React, { useState } from 'react';
function Modal() { const [isOpen, setIsOpen] = useState(false); return(<div><button onClick={()=>setIsOpen(true)}>Open Modal</button>{isOpen && <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{background:'white',padding:'20px',borderRadius:'10px',color:'black'}}><h2>Modal Title</h2><p>Modal Content</p><button onClick={()=>setIsOpen(false)}>Close</button></div></div>}</div>);}
export default Modal;`,
      css: `body{background:#060d1a;color:white;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Modal</title></head><body><div id="root"></div></body></html>`
    },
    "Search Filter": { label: "Search Filter ⚛️", language: "react", jsx: `// Search Filter
import React, { useState } from 'react';
function SearchFilter() { const items = ['Apple','Banana','Orange','Mango','Grapes']; const [query, setQuery] = useState(''); const filtered = items.filter(i=>i.toLowerCase().includes(query.toLowerCase())); return(<div><input type="text" placeholder="Search..." value={query} onChange={e=>setQuery(e.target.value)} style={{padding:'10px',width:'100%'}}/><ul>{filtered.map(i=><li key={i}>{i}</li>)}</ul></div>);}
export default SearchFilter;`,
      css: `body{background:#060d1a;color:white;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Search Filter</title></head><body><div id="root"></div></body></html>`
    },
    "Infinite Scroll": { label: "Infinite Scroll ⚛️", language: "react", jsx: `// Infinite Scroll
import React, { useState, useEffect } from 'react';
function InfiniteScroll() { const [items, setItems] = useState([]); const [page, setPage] = useState(1); useEffect(()=>{fetch(\`https://jsonplaceholder.typicode.com/posts?_limit=10&_page=\${page}\`).then(res=>res.json()).then(data=>setItems(prev=>[...prev,...data]));},[page]); useEffect(()=>{window.addEventListener('scroll',()=>{if(window.innerHeight+window.scrollY>=document.body.offsetHeight-500) setPage(p=>p+1);});},[]); return(<div>{items.map(i=><div key={i.id} style={{border:'1px solid #ccc',margin:'10px',padding:'10px'}}><h3>{i.title}</h3><p>{i.body}</p></div>)}</div>);}
export default InfiniteScroll;`,
      css: `body{background:#060d1a;color:white;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Infinite Scroll</title></head><body><div id="root"></div></body></html>`
    },
    "Star Rating": { label: "Star Rating ⚛️", language: "react", jsx: `// Star Rating
import React, { useState } from 'react';
function StarRating() { const [rating, setRating] = useState(0); return(<div>{[1,2,3,4,5].map(star=><span key={star} onClick={()=>setRating(star)} style={{cursor:'pointer',fontSize:'40px',color:star<=rating?'gold':'gray'}}>★</span>)}</div>);}
export default StarRating;`,
      css: `body{background:#060d1a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Star Rating</title></head><body><div id="root"></div></body></html>`
    },
    "Progress Bar": { label: "Progress Bar ⚛️", language: "react", jsx: `// Progress Bar
import React, { useState } from 'react';
function ProgressBar() { const [progress, setProgress] = useState(0); return(<div><div style={{width:'100%',background:'#ccc',borderRadius:'5px'}}><div style={{width:progress+'%',background:'#38bdf8',padding:'5px',color:'white',textAlign:'center',borderRadius:'5px'}}>{progress}%</div></div><button onClick={()=>setProgress(p=>Math.min(100,p+10))}>Increase</button><button onClick={()=>setProgress(p=>Math.max(0,p-10))}>Decrease</button></div>);}
export default ProgressBar;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Progress Bar</title></head><body><div id="root"></div></body></html>`
    },
    "Dropdown Menu": { label: "Dropdown Menu ⚛️", language: "react", jsx: `// Dropdown Menu
import React, { useState } from 'react';
function Dropdown() { const [isOpen, setIsOpen] = useState(false); const [selected, setSelected] = useState('Select'); const options = ['Option 1','Option 2','Option 3']; return(<div><button onClick={()=>setIsOpen(!isOpen)}>{selected}</button>{isOpen && <ul style={{listStyle:'none',padding:0,background:'white',color:'black',position:'absolute'}}>{options.map(opt=><li key={opt} onClick={()=>{setSelected(opt);setIsOpen(false);}} style={{padding:'5px 10px',cursor:'pointer'}}>{opt}</li>)}</ul>}</div>);}
export default Dropdown;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Dropdown</title></head><body><div id="root"></div></body></html>`
    },
    "Tooltip": { label: "Tooltip ⚛️", language: "react", jsx: `// Tooltip
import React, { useState } from 'react';
function Tooltip() { const [visible, setVisible] = useState(false); return(<div style={{position:'relative'}}><button onMouseEnter={()=>setVisible(true)} onMouseLeave={()=>setVisible(false)}>Hover me</button>{visible && <div style={{position:'absolute',background:'black',color:'white',padding:'5px',borderRadius:'5px',top:'-30px'}}>Tooltip text</div>}</div>);}
export default Tooltip;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Tooltip</title></head><body><div id="root"></div></body></html>`
    },
    "Accordion": { label: "Accordion ⚛️", language: "react", jsx: `// Accordion
import React, { useState } from 'react';
function Accordion() { const [open, setOpen] = useState(false); return(<div><button onClick={()=>setOpen(!open)}>Toggle Section</button>{open && <div>Accordion content here</div>}</div>);}
export default Accordion;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Accordion</title></head><body><div id="root"></div></body></html>`
    },
    "Color Picker": { label: "Color Picker ⚛️", language: "react", jsx: `// Color Picker
import React, { useState } from 'react';
function ColorPicker() { const [color, setColor] = useState('#38bdf8'); return(<div><input type="color" value={color} onChange={e=>setColor(e.target.value)}/><div style={{width:'100px',height:'100px',background:color,marginTop:'10px'}}></div></div>);}
export default ColorPicker;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Color Picker</title></head><body><div id="root"></div></body></html>`
    },
    "Stopwatch": { label: "Stopwatch ⚛️", language: "react", jsx: `// Stopwatch
import React, { useState, useEffect } from 'react';
function Stopwatch() { const [time, setTime] = useState(0); const [running, setRunning] = useState(false); useEffect(()=>{let interval; if(running) interval=setInterval(()=>setTime(t=>t+1),10); return()=>clearInterval(interval);},[running]); return(<div><h1>{(time/100).toFixed(2)}s</h1><button onClick={()=>setRunning(true)}>Start</button><button onClick={()=>setRunning(false)}>Stop</button><button onClick={()=>{setRunning(false);setTime(0);}}>Reset</button></div>);}
export default Stopwatch;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Stopwatch</title></head><body><div id="root"></div></body></html>`
    },
    "Pagination": { label: "Pagination ⚛️", language: "react", jsx: `// Pagination
import React, { useState } from 'react';
function Pagination() { const items = Array.from({length:50},(_,i)=>i+1); const [page, setPage] = useState(1); const perPage=10; const totalPages=Math.ceil(items.length/perPage); const currentItems=items.slice((page-1)*perPage,page*perPage); return(<div>{currentItems.map(i=><div key={i}>{i}</div>)}<div>{Array.from({length:totalPages},(_,i)=><button key={i} onClick={()=>setPage(i+1)} style={{margin:'5px'}}>{i+1}</button>)}</div></div>);}
export default Pagination;`,
      css: `body{background:#060d1a;color:white;padding:40px;}`,
      html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pagination</title></head><body><div id="root"></div></body></html>`
    }
  },
  HTML_CSS_JS: {
    ...
    "Hover Board": { label: "Hover Board 🎨", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Hover Board</title><link rel="stylesheet" href="styles.css"></head><body><div class="board"></div></body></html>`, css: `*{margin:0;padding:0;box-sizing:border-box;}body{min-height:100vh;background:#111;display:flex;align-items:center;justify-content:center;}.board{display:grid;grid-template-columns:repeat(20,30px);gap:2px;}.square{width:30px;height:30px;background:#1d1d1d;transition:0.1s;}.square:hover{transition:0s;background:radial-gradient(circle,#ff6b6b,#4ecdc4);box-shadow:0 0 10px #4ecdc4;}`, js: `const board=document.querySelector('.board');for(let i=0;i<400;i++){const square=document.createElement('div');square.classList.add('square');square.addEventListener('mouseover',()=>{const colors=['#ff6b6b','#4ecdc4','#45b7d1','#96ceb4'];square.style.background=colors[Math.floor(Math.random()*colors.length)];setTimeout(()=>{square.style.background='#1d1d1d';},300);});board.appendChild(square);}` },
    "Drag and Drop": { label: "Drag and Drop 📦", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Drag and Drop</title><link rel="stylesheet" href="styles.css"></head><body><div class="draggable" draggable="true">Drag me</div><div class="dropzone">Drop here</div></body></html>`, css: `.draggable{width:100px;height:100px;background:#38bdf8;margin:20px;display:flex;align-items:center;justify-content:center;cursor:grab;}.dropzone{width:200px;height:200px;background:#333;margin:20px;display:flex;align-items:center;justify-content:center;color:white;}`, js: `const draggable=document.querySelector('.draggable');const dropzone=document.querySelector('.dropzone');draggable.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain','dragged');});dropzone.addEventListener('dragover',e=>{e.preventDefault();});dropzone.addEventListener('drop',e=>{e.preventDefault();dropzone.style.background='#38bdf8';dropzone.textContent='Dropped!';});` },
    "Image Slider": { label: "Image Slider 🖼️", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Image Slider</title><link rel="stylesheet" href="styles.css"></head><body><div class="slider"><img id="sliderImg" src="https://picsum.photos/400/300?random=1"><button id="prev">Prev</button><button id="next">Next</button></div></body></html>`, css: `.slider{text-align:center;}img{width:400px;height:300px;object-fit:cover;}button{padding:10px 20px;margin:10px;}`, js: `const images=['https://picsum.photos/400/300?random=1','https://picsum.photos/400/300?random=2','https://picsum.photos/400/300?random=3'];let idx=0;const img=document.getElementById('sliderImg');document.getElementById('prev').addEventListener('click',()=>{idx=(idx-1+images.length)%images.length;img.src=images[idx];});document.getElementById('next').addEventListener('click',()=>{idx=(idx+1)%images.length;img.src=images[idx];});` },
    "Typing Effect": { label: "Typing Effect ⌨️", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Typing Effect</title><link rel="stylesheet" href="styles.css"></head><body><h1 id="typed"></h1></body></html>`, css: `body{background:#060d1a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:monospace;}`, js: `const text="Welcome to Typing Effect!";let i=0;function type(){if(i<text.length){document.getElementById('typed').innerHTML+=text.charAt(i);i++;setTimeout(type,100);}}type();` },
    ...
    "Weather Widget": { label: "Weather Widget 🌤️", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Weather Widget</title><link rel="stylesheet" href="styles.css"></head><body><div class="weather"><h2>Weather</h2><div id="temp">--°C</div><div id="condition">Loading...</div></div></body></html>`, css: `.weather{background:linear-gradient(135deg,#667eea,#764ba2);padding:30px;border-radius:20px;color:white;text-align:center;}`, js: `async function getWeather(){try{const res=await fetch('https://api.open-meteo.com/v1/forecast?latitude=35.6895&longitude=139.6917&current_weather=true');const data=await res.json();document.getElementById('temp').textContent=data.current_weather.temperature+'°C';document.getElementById('condition').textContent=data.current_weather.weathercode===0?'Clear':'Cloudy';}catch(e){document.getElementById('condition').textContent='API Error';}}getWeather();` },
    "Quiz App": { label: "Quiz App ❓", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Quiz App</title><link rel="stylesheet" href="styles.css"></head><body><div class="quiz"><h2>Quiz</h2><div id="question"></div><div id="options"></div><button id="next">Next</button></div></body></html>`, css: `.quiz{background:#07101f;padding:30px;border-radius:20px;color:white;max-width:500px;margin:auto;margin-top:20vh;}button{margin-top:20px;padding:10px 20px;background:#38bdf8;border:none;border-radius:8px;cursor:pointer;}`, js: `const questions=[{q:"What is React?",o:["Library","Framework","Language"],a:0},{q:"JSX stands for?",o:["JavaScript XML","Java XML","JS Extension"],a:0}];let idx=0,score=0;function load(){const q=questions[idx];document.getElementById('question').textContent=q.q;const optsDiv=document.getElementById('options');optsDiv.innerHTML='';q.o.forEach((opt,i)=>{const btn=document.createElement('button');btn.textContent=opt;btn.onclick=()=>{if(i===q.a)score++;idx++;if(idx<questions.length)load();else alert('Quiz complete! Score:'+score);};optsDiv.appendChild(btn);});}load();` },
    ...
    "Notes App": { label: "Notes App 📝", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Notes App</title><link rel="stylesheet" href="styles.css"></head><body><div class="notes"><h2>Notes</h2><textarea id="note" placeholder="Write your note..."></textarea><button id="save">Save</button><div id="savedNotes"></div></div></body></html>`, css: `.notes{max-width:500px;margin:auto;margin-top:20vh;background:#07101f;padding:20px;border-radius:20px;color:white;}textarea{width:100%;height:100px;margin:10px 0;}button{padding:10px 20px;background:#38bdf8;border:none;border-radius:8px;cursor:pointer;}`, js: `let notes=JSON.parse(localStorage.getItem('notes')||'[]');function render(){const div=document.getElementById('savedNotes');div.innerHTML='';notes.forEach((note,i)=>{const p=document.createElement('p');p.textContent=note;p.innerHTML+=' <button onclick="deleteNote('+i+')">Delete</button>';div.appendChild(p);});}function deleteNote(i){notes.splice(i,1);localStorage.setItem('notes',JSON.stringify(notes));render();}document.getElementById('save').addEventListener('click',()=>{const note=document.getElementById('note').value;if(note){notes.push(note);localStorage.setItem('notes',JSON.stringify(notes));render();document.getElementById('note').value='';}});render();` },
    "Password Generator": { label: "Password Generator 🔐", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Password Generator</title><link rel="stylesheet" href="styles.css"></head><body><div class="pwd"><h2>Password Generator</h2><input id="password" readonly><button id="generate">Generate</button></div></body></html>`, css: `.pwd{text-align:center;margin-top:20vh;background:#07101f;padding:30px;border-radius:20px;max-width:400px;margin-left:auto;margin-right:auto;color:white;}input{width:100%;padding:10px;margin:10px 0;font-size:16px;}button{padding:10px 20px;background:#38bdf8;border:none;border-radius:8px;cursor:pointer;}`, js: `document.getElementById('generate').addEventListener('click',()=>{const chars='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*';let pwd='';for(let i=0;i<12;i++)pwd+=chars[Math.floor(Math.random()*chars.length)];document.getElementById('password').value=pwd;});` },
    "Toast Notification": { label: "Toast Notification 🔔", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Toast</title><link rel="stylesheet" href="styles.css"></head><body><button id="showToast">Show Toast</button><div id="toast" class="toast">Hello World!</div></body></html>`, css: `.toast{visibility:hidden;min-width:250px;background:#333;color:white;text-align:center;border-radius:8px;padding:16px;position:fixed;bottom:30px;left:50%;transform:translateX(-50%);z-index:1;}.toast.show{visibility:visible;animation:fadein 0.5s,fadeout 0.5s 2.5s;}@keyframes fadein{from{bottom:0;opacity:0;}to{bottom:30px;opacity:1;}}@keyframes fadeout{from{bottom:30px;opacity:1;}to{bottom:0;opacity:0;}}`, js: `function showToast(){const toast=document.getElementById('toast');toast.className='toast show';setTimeout(()=>{toast.className='toast';},3000);}document.getElementById('showToast').addEventListener('click',showToast);` },
    "Rock Paper Scissors": { label: "Rock Paper Scissors ✊", language: "htmlcssjs", html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>RPS</title><link rel="stylesheet" href="styles.css"></head><body><div class="game"><h2>Rock Paper Scissors</h2><button id="rock">Rock</button><button id="paper">Paper</button><button id="scissors">Scissors</button><div id="result"></div></div></body></html>`, css: `.game{text-align:center;margin-top:20vh;background:#07101f;padding:30px;border-radius:20px;color:white;}button{margin:10px;padding:10px 20px;background:#38bdf8;border:none;border-radius:8px;cursor:pointer;}`, js: `const choices=['Rock','Paper','Scissors'];document.querySelectorAll('.game button').forEach(btn=>{btn.addEventListener('click',(e)=>{const player=e.target.id;const comp=choices[Math.floor(Math.random()*3)];let result='';if(player===comp)result='Tie!';else if((player==='rock'&&comp==='Scissors')||(player==='paper'&&comp==='Rock')||(player==='scissors'&&comp==='Paper'))result='You win!';else result='You lose!';document.getElementById('result').textContent=You chose ${player}, Computer chose ${comp}. ${result};});});` },
...
run();` },
    "Fetch API": { label: "Fetch API 🌐", language: "javascript", code: `// Fetch API
fetch('https://jsonplaceholder.typicode.com/posts/1')
  .then(res=>res.json())
  .then(data=>console.log(data))
  .catch(err=>console.error(err));` },
    "LocalStorage": { label: "LocalStorage 💾", language: "javascript", code: `// LocalStorage Demo
localStorage.setItem('test','Hello World');
console.log(localStorage.getItem('test'));
localStorage.removeItem('test');` },
    "Event Loop": { label: "Event Loop 🔄", language: "javascript", code: `// Event Loop Demo
console.log('1');
setTimeout(()=>console.log('2'),0);
Promise.resolve().then(()=>console.log('3'));
console.log('4'); // Output: 1,4,3,2` },
    "Classes": { label: "ES6 Classes 🏗️", language: "javascript", code: `// Class Demo
class Animal{constructor(name){this.name=name;}speak(){console.log(${this.name} makes a sound);}}
class Dog extends Animal{speak(){console.log(${this.name} barks);}}
const d=new Dog('Rex');d.speak();` },
    "Spread Operator": { label: "Spread Operator ...", language: "javascript", code: `// Spread Operator
const arr1=[1,2,3];const arr2=[4,5,6];
const combined=[...arr1,...arr2];
console.log(combined);
const obj={a:1,b:2};const copy={...obj,c:3};
console.log(copy);` },
    "Destructuring": { label: "Destructuring 📦", language: "javascript", code: `// Destructuring
...
console.log(first,second);` },
    "Map & Set": { label: "Map & Set 🗺️", language: "javascript", code: `// Map & Set
const map=new Map();map.set('key','value');
console.log(map.get('key'));
const set=new Set([1,2,2,3]);console.log(set);` },
    "Generator": { label: "Generator 🔁", language: "javascript", code: `// Generator Function
...
const gen=idGenerator();console.log(gen.next().value);console.log(gen.next().value);` },
    "Proxy": { label: "Proxy 🛡️", language: "javascript", code: `// Proxy Demo
const target={message:'hello'};
const handler={get:(obj,prop)=>prop in obj?obj[prop]:'default'};
const proxy=new Proxy(target,handler);
console.log(proxy.message);console.log(proxy.unknown);` },
    "WebSocket": { label: "WebSocket 🔌", language: "javascript", code: `// WebSocket Demo (simulated)
console.log('WebSocket would connect to ws://example.com');
// const ws = new WebSocket('ws://example.com');` },
    "Service Worker": { label: "Service Worker 📡", language: "javascript", code: `// Service Worker Registration
if('serviceWorker' in navigator){
  navigator.serviceWorker.register('/sw.js').then(reg=>console.log('Registered',reg));
}` },
    "Intersection Observer": { label: "Intersection Observer 👁️", language: "javascript", code: `// Intersection Observer
const observer=new IntersectionObserver(entries=>entries.forEach(e=>console.log(e.isIntersecting?'Visible':'Hidden')));
observer.observe(document.body);` },
    "Drag and Drop": { label: "Drag and Drop 🖱️", language: "javascript", code: `// HTML5 Drag and Drop
function allowDrop(e){e.preventDefault();}
function drop(e){e.preventDefault();const data=e.dataTransfer.getData('text');e.target.appendChild(document.getElementById(data));}
console.log('Drag and drop handlers ready');` },
    "Canvas": { label: "Canvas 🎨", language: "javascript", code: `// Canvas Drawing
const canvas=document.createElement('canvas');canvas.width=200;canvas.height=200;
const ctx=canvas.getContext('2d');ctx.fillStyle='red';ctx.fillRect(0,0,200,200);
document.body.appendChild(canvas);` },
    "Audio": { label: "Audio 🎵", language: "javascript", code: `// Web Audio
const audio=new Audio('https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3');
audio.play().catch(e=>console.log('Autoplay blocked'));` },
    "Geolocation": { label: "Geolocation 📍", language: "javascript", code: `// Geolocation
...
console.log(greetUser(user));` },
    "Generics": { label: "Generics 🔧", language: "typescript", code: `// Generics
function identity<T>(arg:T):T{return arg;}
console.log(identity<string>('Hello'));console.log(identity<number>(42));` },
    "Union Types": { label: "Union Types 🔀", language: "typescript", code: `// Union Types
type ID = string | number;
function printId(id:ID){console.log(Your ID is: ${id});}
printId(101);printId('abc');` },
    "Tuples": { label: "Tuples 📦", language: "typescript", code: `// Tuples
let tuple:[string,number]=['Alice',30];
console.log(tuple[0],tuple[1]);` },
    "Enums": { label: "Enums 🏷️", language: "typescript", code: `// Enums
enum Color{Red,Green,Blue}
let c:Color=Color.Green;
console.log(c);` },
    "Async/Await": { label: "Async/Await ⏳", language: "typescript", code: `// Async/Await
...
def fib(n):
    ...
    for _ in range(n):
        ...
        a,b=b,a+b
fib(10)` },
    "List Comprehension": { label: "List Comprehension 📝", language: "python", code: `# List Comprehension
squares = [x**2 for x in range(10)]
print(f"Squares: {squares}")` },
    "Dictionary": { label: "Dictionary 📚", language: "python", code: `# Dictionary Demo
d = {'name':'Alice','age':30}
print(d['name'])
for k,v in d.items(): print(f"{k}: {v}")` },
    "File I/O": { label: "File I/O 📁", language: "python", code: `# File Operations
...
SELECT * FROM users;` },
    "Joins": { label: "JOINS 🔗", language: "sql", code: `-- JOIN Demo
SELECT u.name, o.amount FROM users u JOIN orders o ON u.id=o.user_id;` },
    "Group By": { label: "GROUP BY 📊", language: "sql", code: `-- GROUP BY
...
echo "User: $(whoami)"
echo "Date: $(date)"` },
    "File Operations": { label: "File Ops 📁", language: "bash", code: `#!/bin/bash
mkdir test_dir
...
public class Main {
    ...
    "FizzBuzz": { label: "FizzBuzz 🔢", language: "java", code: `// FizzBuzz
public class Main {
    public static void main(String[] args) {
        for(int i=1;i<=15;i++){
...
int main() {
    ...
    "Sum": { label: "Sum of Numbers ➕", language: "cpp", code: `// Sum
#include <iostream>
using namespace std;
int main() {
    int sum=0;
...
int main() {
    ...
    "Factorial": { label: "Factorial 🔢", language: "c", code: `// Factorial
#include <stdio.h>
