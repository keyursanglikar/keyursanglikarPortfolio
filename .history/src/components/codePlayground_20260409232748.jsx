...
const generateOutputHTML = useCallback(() => {
  if (currentLanguage === "react") {
    // Clean the JSX code - remove export default and fix imports
    let cleanJsxCode = jsxCode;
    
    // Remove import statements (they don't work in browser)
    cleanJsxCode = cleanJsxCode.replace(/import\s+.*?from\s+['"][^'"]+['"]\s*;?\n?/g, '');
    
    // Remove export default and replace with variable declaration
    if (cleanJsxCode.includes('export default')) {
      const match = cleanJsxCode.match(/export\s+default\s+(\w+)/);
      if (match) {
        cleanJsxCode = cleanJsxCode.replace(/export\s+default\s+\w+/, `const App = ${match[1]}`);
      } else {
        cleanJsxCode = cleanJsxCode.replace('export default', 'const App =');
      }
    }
    
    // Ensure there's a main component
    let mainComponent = '';
    const functionMatch = cleanJsxCode.match(/function\s+(\w+)\s*\(/);
    const constMatch = cleanJsxCode.match(/const\s+(\w+)\s*=\s*\(/);
    
    if (functionMatch) {
      mainComponent = functionMatch[1];
    } else if (constMatch) {
      mainComponent = constMatch[1];
    } else {
      mainComponent = 'App';
      // Wrap the code in a default App component if none exists
      if (!cleanJsxCode.includes('function App') && !cleanJsxCode.includes('const App')) {
        cleanJsxCode = `
function App() {
  ${cleanJsxCode}
  return <div>${cleanJsxCode.replace(/[\n\r]/g, '<br/>')}</div>;
}
`;
      }
    }
...
<body>
  ...
  <script type="text/babel">
    ...
    const ComponentToRender = ${mainComponent};
    ...
    if (htmlCssJsCode.css) {
      const styleTag = `<style>${htmlCssJsCode.css}</style>`;
    ...
    if (htmlCssJsCode.js) {
      const scriptTag = `<script>${htmlCssJsCode.js}</script>`;
    ...
    // For non-web languages, show code preview
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${currentLanguage.toUpperCase()} Code Preview</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  <div class="container">
    <h1>📝 ${currentLanguage.toUpperCase()} Code</h1>
    <pre>${plainCode.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre>
    <div class="note">
      ⚡ This is a code preview. ${currentLanguage === 'javascript' || currentLanguage === 'typescript' ? 'You can run this code in your browser console.' : 'Run this code in your local environment for execution.'}
    </div>
  </div>
</body>
</html>`;
